...
describe('alphaVantage adapter', () => {
  ...
  it('computes assetGrowth and ebitdaGrowth from BALANCE_SHEET and INCOME_STATEMENT (Task #38)', async () => {
    vi.doMock('axios', () => ({
      default: {
        get: vi.fn().mockImplementation((url: string, opts: any) => {
          const fn = opts.params.function;
          if (fn === 'OVERVIEW') {
            return Promise.resolve({
              data: {
                Symbol: 'AAPL', Name: 'Apple Inc.',
                EPS: '6.10', PERatio: '28.50',
                OperatingCashflowPerShare: '7.20',
                ReturnOnEquityTTM: '0.45', ProfitMargin: '0.25',
                OperatingMarginTTM: '0.30', '52WeekHigh': '199.61', '52WeekLow': '164.08',
              },
            });
          }
          if (fn === 'INCOME_STATEMENT') {
            return Promise.resolve({
              data: {
                annualReports: [
                  { ebitda: '120000000000' }, // current year
                  { ebitda: '100000000000' }, // prior year
                ],
              },
            });
          }
          if (fn === 'BALANCE_SHEET') {
            return Promise.resolve({
              data: {
                annualReports: [
                  { totalAssets: '352000000000' }, // current year
                  { totalAssets: '320000000000' }, // prior year
                ],
              },
            });
          }
          // GLOBAL_QUOTE
          return Promise.resolve({ data: { 'Global Quote': { '05. price': '170.25' } } });
        }),
      },
    }));
    ...
    const result = await getAlphaVantageData('AAPL');
    expect(stockResponseSchema.safeParse(result).success).toBe(true);
    // ebitdaGrowth = (120B - 100B) / 100B * 100 = 20%
    expect(result.multibaggerSignals?.ebitdaGrowth).toBeCloseTo(20, 1);
    // assetGrowth = (352B - 320B) / 320B * 100 = 10%
    expect(result.multibaggerSignals?.assetGrowth).toBeCloseTo(10, 1);
    // fcfYield remains null (not available from AV free tier)
    expect(result.multibaggerSignals?.fcfYield).toBeNull();
  });

  it('leaves assetGrowth/ebitdaGrowth null when financial statements have < 2 reports', async () => {
    vi.doMock('axios', () => ({
      default: {
        get: vi.fn().mockImplementation((url: string, opts: any) => {
          const fn = opts.params.function;
          if (fn === 'OVERVIEW') {
            return Promise.resolve({
              data: {
                Symbol: 'NEW', Name: 'New Co',
                EPS: '2.00', ReturnOnEquityTTM: '0.10',
                ProfitMargin: '0.05', OperatingMarginTTM: '0.05',
              },
            });
          }
          if (fn === 'INCOME_STATEMENT') {
            return Promise.resolve({ data: { annualReports: [{ ebitda: '5000000000' }] } }); // only 1 year
          }
          if (fn === 'BALANCE_SHEET') {
            return Promise.resolve({ data: { annualReports: [] } }); // empty
          }
          return Promise.resolve({ data: { 'Global Quote': { '05. price': '50.00' } } });
        }),
      },
    }));

    const { getAlphaVantageData } = await import('../server/services/alphaVantage');
    const result = await getAlphaVantageData('NEW');
    expect(stockResponseSchema.safeParse(result).success).toBe(true);
    expect(result.multibaggerSignals?.ebitdaGrowth).toBeNull();
