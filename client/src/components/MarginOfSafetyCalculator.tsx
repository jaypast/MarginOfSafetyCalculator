...
const MarginOfSafetyCalculator: React.FC = () => {
  ...
  const [valuationResultSymbol, setValuationResultSymbol] = useState<string | null>(null);
  const [successfulSearchAttempts, setSuccessfulSearchAttempts] = useState(0);
  ...
  const lastTrackedValuationAttempt = React.useRef(0);
  ...
  const calculateIntrinsicValue = () => {
    ...
    setValuationResultSymbol(stockData.symbol);
  ...
  }, [marginOfSafetyParams, stockData, valuationParams]);

  const hasResults =
    stockData
    && !stockData.error
    && valuationResultSymbol === stockData.symbol
    && valuationResults.length > 0;
  const activeResultMethod = {
    dcf: 'DCF Analysis',
    pe: 'P/E Based',
    graham: 'Graham Formula',
  }[activeMethod];
  const activeResult = valuationResults.find(
    (result) => result.method === activeResultMethod,
  );
  const valuationOutcome = classifyValuationOutcome(
    activeResult?.intrinsicValue,
    activeResult?.discountPremium,
  );
  const handleSearchCompleted = React.useCallback(
    (outcome: 'success' | 'error') => {
      if (outcome === 'success') {
        setSuccessfulSearchAttempts((attempts) => attempts + 1);
      }
    },
    [],
  );
  ...
  useEffect(() => {
    if (
      ...
      || stockData.error
      || !hasResults
      || successfulSearchAttempts <= lastTrackedValuationAttempt.current
    ) {
      return;
    }
    lastTrackedValuationAttempt.current = successfulSearchAttempts;

    trackDecisionFunnelEvent('valuation_completed', {
      method: activeMethod,
