      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <div className="text-right shrink-0">
            <div
              className={`inline-flex flex-col items-center px-3 py-2 rounded-lg border ${TONE_CLASSES[tone]}`}
              data-testid="screener-composite"
            >
              <span className="text-2xl font-bold tabular-nums leading-none">
                {result.composite === null ? '—' : result.composite.toFixed(0)}
              </span>
              <span className="text-[10px] uppercase tracking-wide mt-1">{band.label}</span>
            </div>
          </div>
        </div>
        ...
        {result.missingFactors.length > 0 && (
          <div className="mt-2 text-[11px] text-neutral-500">
            ...
            Composite renormalised across {result.subScores.length - result.missingFactors.length}/
